...
def can_be_created_in_a_block(args = {title: "Home Alone", release_date: 1990})
  ...
  Movie.create do |m|
    args.each { |key, value| m[key] = value }
  end
end
...
def can_be_found_updated_and_saved
  ...
  awesome_flick.title = 'Even Awesomer Flick'
  awesome_flick.save
  awesome_flick
end
...
def can_update_using_update_method
  ...
  wat = Movie.find_by(title: 'Wat?')
  wat.update(title: "Wat, huh?")
end
...
def can_destroy_a_single_item
  ...
  movie_to_delete = Movie.find_by(title: 'That One Where the Guy Kicks Another Guy Once')
